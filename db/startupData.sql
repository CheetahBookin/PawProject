INSERT INTO Accomodation (name, address, country, city, type, carParkFee)
VALUES 
('Grand Plaza Hotel', '123 Main St', 'USA', 'New York', 'Hotel', 10.50),
('Ocean Breeze Motel', '456 Elm St', 'USA', 'Los Angeles', 'Motel', NULL),
('Tropical Paradise Resort', '789 Oak St', 'USA', 'Miami', 'Resort', 15.75),
('The Royal Hotel', '101 Pine St', 'UK', 'London', 'Hotel', 8.25),
('Urban Backpackers Hostel', '202 Maple St', 'UK', 'Manchester', 'Hostel', 5.00),
('Cozy Corner Guesthouse', '303 Birch St', 'UK', 'Birmingham', 'Guesthouse', NULL),
('Eiffel Tower Hotel', '404 Cedar St', 'France', 'Paris', 'Hotel', 12.00),
('Seaside Sunset Motel', '505 Walnut St', 'France', 'Marseille', 'Motel', NULL),
('Riviera Resort', '606 Spruce St', 'France', 'Nice', 'Resort', 18.50),
('Bavarian Flat', '707 Cherry St', 'Germany', 'Berlin', 'Flat', 9.75),
('Alpine Adventure Hostel', '808 Pine St', 'Germany', 'Munich', 'Hostel', 6.50),
('Black Forest Guesthouse', '909 Elm St', 'Germany', 'Hamburg', 'Guesthouse', NULL),
('Sunny Madrid Hotel', '111 Oak St', 'Spain', 'Madrid', 'Hotel', 11.25),
('Barcelona Beach Motel', '222 Maple St', 'Spain', 'Barcelona', 'Motel', NULL),
('Costa del Sol Resort', '333 Birch St', 'Spain', 'Valencia', 'Resort', 16.00),
('Hotel NH Poznań', 'Święty Marcin 67', 'Poland', 'Poznań', 'Hotel', 10.00),
('Hotel Morena', 'Konopnickiej 1A', 'Poland', 'Mosina', 'Hotel', 7.00),
('Venetian Dreams Guesthouse', '666 Spruce St', 'Italy', 'Venice', 'Guesthouse', NULL),
('Sydney Harbor Hotel', '777 Cherry St', 'Australia', 'Sydney', 'Hotel', 13.50),
('Melbourne Gateway Motel', '888 Pine St', 'Australia', 'Melbourne', 'Motel', NULL),
('Queensland Oasis Resort', '999 Elm St', 'Australia', 'Brisbane', 'Resort', 20.00),
('Maple Leaf Inn', '121 Oak St', 'Canada', 'Toronto', 'Inn', 12.75),
('Pacific Coast Hostel', '232 Maple St', 'Canada', 'Vancouver', 'Hostel', 8.00),
('Montreal Magic Guesthouse', '343 Birch St', 'Canada', 'Montreal', 'Guesthouse', NULL),
('Tokyo Tower Hotel', '454 Cedar St', 'Japan', 'Tokyo', 'Hotel', 14.25),
('Osaka Oasis Motel', '565 Walnut St', 'Japan', 'Osaka', 'Motel', NULL),
('Kyoto Gardens Resort', '676 Spruce St', 'Japan', 'Kyoto', 'Resort', 22.50),
('Seoul Serenity Inn', '787 Cherry St', 'South Korea', 'Seoul', 'Inn', 13.25),
('Busan Backpackers Hostel', '898 Pine St', 'South Korea', 'Busan', 'Hostel', 9.00),
('Incheon Island Guesthouse', '909 Elm St', 'South Korea', 'Incheon', 'Guesthouse', NULL),
('Forbidden City Hotel', '123 Oak St', 'China', 'Beijing', 'Hotel', 15.75),
('Shanghai Skyline Motel', '234 Maple St', 'China', 'Shanghai', 'Motel', NULL),
('Guangzhou Green Resort', '345 Birch St', 'China', 'Guangzhou', 'Resort', 25.00),
('Bollywood Inn', '456 Cedar St', 'India', 'Mumbai', 'Inn', 14.50),
('Delhi Daze Hostel', '567 Walnut St', 'India', 'New Delhi', 'Hostel', 10.00),
('Bangalore Bliss Guesthouse', '678 Spruce St', 'India', 'Bangalore', 'Guesthouse', NULL),
('Sao Paulo Splendor Hotel', '789 Cherry St', 'Brazil', 'Sao Paulo', 'Hotel', 17.25),
('Rio Retreat Motel', '890 Pine St', 'Brazil', 'Rio de Janeiro', 'Motel', NULL),
('Brasilia Bay Resort', '901 Elm St', 'Brazil', 'Brasilia', 'Resort', 27.50),
('Mexico City Lights Inn', '112 Oak St', 'Mexico', 'Mexico City', 'Inn', 15.00),
('Cancun Cove Hostel', '223 Maple St', 'Mexico', 'Cancun', 'Hostel', 11.50),
('Guadalajara Gem Guesthouse', '334 Birch St', 'Mexico', 'Guadalajara', 'Guesthouse', NULL),
('Moscow Manor Hotel', '445 Cedar St', 'Russia', 'Moscow', 'Hotel', 18.75),
('Saint Petersburg Stopover Motel', '556 Walnut St', 'Russia', 'Saint Petersburg', 'Motel', NULL),
('Sochi Sunset Resort', '667 Spruce St', 'Russia', 'Sochi', 'Resort', 30.00);
('Luxury Grand Hotel', '123 Broadway St', 'USA', 'Chicago', 'Hotel', 20.00),
('Seaside Resort Hotel', '456 Ocean Ave', 'USA', 'San Francisco', 'Hotel', 15.00),
('Mountain View Hotel', '789 Summit Rd', 'USA', 'Denver', 'Hotel', 10.00),
('Historic Downtown Hotel', '101 Main St', 'USA', 'Boston', 'Hotel', 18.00),
('Skyline View Hotel', '202 High St', 'USA', 'Seattle', 'Hotel', 12.00);

INSERT INTO AccomodationImages (hotelId, image)
VALUES 
(1, 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/227628314.jpg?k=97943a69cdb871b4a0063770f6dc9119c953dbddc663830f94f09e786d4bb54f&o=&hp=1'),
(1, 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/52735814.jpg?k=8f97d0bde5b798604079982c90aae8278047eebce3a9a88e2019cda9f4396680&o=&hp=1'),
(1, 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/151948526.jpg?k=0c77f8752ed65cb75ed7107d12c9f7e4173d5bd85a83687e408b56fa7d2714ae&o=&hp=1'),

(2, 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/522803448.jpg?k=6c15cddfcd6ecc30094d238f754f6f19bdc20a6da18a4f9d049bbb2eb2195e15&o=&hp=1'),
(2, 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/340435196.jpg?k=54af1720504492fbaece85519830d12015b99075fc293528846c14449e36e05a&o=&hp=1'),
(2, 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/202488542.jpg?k=73a6794b61a60636e47ccd3661842ee38a90b957d0a4a2186f115abef384ae06&o=&hp=1'),

(3, 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/491539039.jpg?k=c0ccfa4d2adab6fab0222a7caf9e61040a6d043baf39cf6cc2d6949e0bf9e994&o=&hp=1'),
(3, 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/445673998.jpg?k=d6d011cd4266ce2eeee0c0056b7e9ccb57fb77a051e6198bb51c360d8a5f228a&o=&hp=1'),
(3, 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/485476283.jpg?k=44e090ad207bc736ff4a92161ea9454b35b391cc6ec4ea3a2482053eb8dfd407&o=&hp=1'),

(4, 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/329065860.jpg?k=3f1ece75b2b3e5f3199ef79e4786dc55ab2a9cc2e3ee444f9d5b9806551dc464&o=&hp=1'),
(4, 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/493258222.jpg?k=703902bf4974881cdce01a138383ac517679b2faab3695c7b50ca21c12a88683&o=&hp=1'),
(4, 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/493258218.jpg?k=5f51a8e56a2c8db3f689d358b4388a7eb00578c3d19cdc702619f0cff3488375&o=&hp=1'),

(5, 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/308269357.jpg?k=754bcf37a945730edb7c476cfb78f4bcd54b5ee12594453df944a3f36c4339dd&o=&hp=1'),
(5, 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/234993196.jpg?k=df85ea350af25af6dda3baa5e2a212177f9f102158878721e5e5a1a0c843b7fb&o=&hp=1'),
(5, 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/228769339.jpg?k=681ab89b1ce7ab3a58e5949ea7a392ab2e08d40a89b7a849f601a9413a05646b&o=&hp=1'),

(6, 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/523728136.jpg?k=40d22b090c4f413104a1af292e6a1cc64205112d218008d35889e87b8f92c628&o=&hp=1'),
(6, 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/499967175.jpg?k=617bf90042ae25681cba09f853dc70e615b7bec6d64e68f377f1d06c54b95f8f&o=&hp=1'),
(6, 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/499967630.jpg?k=20e19c94eea1942239291452ba385c15657d2702569f3af9ad532782b6eb0f48&o=&hp=1'),

(7, 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/37956412.jpg?k=69ba12d680b59b42a0a88136e21173a3f31b406d19422b5f88882b8d5ced414c&o=&hp=1'),
(7, 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/45409896.jpg?k=dd2e593c54af3b3ad718a6c666dfe8416df2247df332b0280d2c81735f341b44&o=&hp=1'),
(7, 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/176888383.jpg?k=eae8ce068149bf87c76509ee1ad5eb13a37e8580a52ec6f7834cedfe86ba132c&o=&hp=1'),

(8, 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/185166142.jpg?k=28077872b648932143ceb2ce0525ab2c6292393f06332dc7c277e43d3a232d67&o=&hp=1'),
(8, 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/478225034.jpg?k=a4ab8383ce1e53d6b875e18f3ef4a0cf679809615b9aa0f29432da8d322f96e6&o=&hp=1'),
(8, 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/478225071.jpg?k=c93552409c61a5154cb380bb4dace73eb0aec5b90976ff9e495e640c9a89bca7&o=&hp=1'),

(9, 'https://lh3.googleusercontent.com/p/AF1QipOsMY6iHWlfbj3DgtKMCg4ZlYYPADbtV0JWlfkb=w287-h192-n-k-no-v1-rj'),
(9, 'https://lh3.googleusercontent.com/p/AF1QipOhkoJs1T0LOMlHgrDbnIDagG4gslRIV_CtWCWP=w287-h192-n-k-no-v1-rj'),
(9, 'https://lh3.googleusercontent.com/p/AF1QipOxQnMBYCXc4GxXn4tdfV38EFZr1Vt974rojgm9=w287-h192-n-k-no-v1-rj'),

(10, 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/221432329.jpg?k=abb347df68159b17648dbfc3824780a884bb9fe4fd4c3b6a4ea613c91469cf1b&o=&hp=1'),
(10, 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/221428893.jpg?k=8bd76f4dcfa4c78cfe986beb6aae9da28a76ae9bfa550068ff5994e6a1a8d1b5&o=&hp=1'),
(10, 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/221429358.jpg?k=d61595b2234d2d1620e337b469fe367f5917acfa337aa13446c12651093d69ee&o=&hp=1'),

(11, 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/5825354.jpg?k=8ce8812bc1af48008b8c28b7048abe255f67905d62b0b1fd3c2f0c66bf4fd3ae&o=&hp=1'),
(11, 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/86863346.jpg?k=c4638de9e0eb9155f29a41db74415e00d9ebbdac9d2b8e4551e3913a1139897f&o=&hp=1'),
(11, 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/89326304.jpg?k=d1cabe74029b6ed39be356b6529b8fa24603be638520bc60d1e03f616050adc6&o=&hp=1'),

(12, 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/177079853.jpg?k=0e4f6f23f165346a9a3c74c402495e7748c3ff8b138e1316092ae4be638725f2&o=&hp=1'),
(12, 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/218311341.jpg?k=884e925ebf5ac0fc316ac719da2a8d3dafd569587faf5975f548e4467c56dcb1&o=&hp=1'),
(12, 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/238556785.jpg?k=4396ef4f239a9a79304e02cdb3251eb7d900a8aad93a45f611a57d8921eb37e1&o=&hp=1'),

(13, 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/145834019.jpg?k=5970ad3b0fc74573bc3bd36fcb450fef3f6d8d14938c1997dad3c46c8352571d&o=&hp=1'),
(13, 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/198523612.jpg?k=3b3eda416e0a7f7a26c697838bafa1633fb0aa1daa49c32c3cd80eabada2de95&o=&hp=1'),
(13, 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/198523574.jpg?k=57907a891e82840b91c760bf666bf6cf4ff814d81840175cb970eb023d0d9a50&o=&hp=1'),

(14, 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/309320834.jpg?k=ba0b996dec3d7b6a0712527865deb392224f3bf4e416a017a5697c4c73b9e8a4&o=&hp=1'),
(14, 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/436245170.jpg?k=933a72f54d04eddb6f6a4a7544abe45e27a48d2ba3c8de78eabe1d729d739adf&o=&hp=1'),
(14, 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/17041981.jpg?k=5dc23bd04cc9c0d2be99aba08f1a996ea34fbed1c0db93d8e97b4af5be5ca8da&o=&hp=1'),

(15, 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/216694650.jpg?k=4be9e9133e2b3c3da2addfbbe7f0626ce020b8249d2266c6eaa672e351b2b531&o=&hp=1'),
(15, 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/216697221.jpg?k=d6dd4a1ea76933c0720514a3ac780af507878c904cbbed6c6c601d1c54644afb&o=&hp=1'),
(15, 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/216694662.jpg?k=f85d7a9be2ef6873bb2df97aedda42d9830fb80a819f3ff32ead9668cac6a7d2&o=&hp=1'),

(16, 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/318821098.jpg?k=599d88e68b3dbd563908f59e71e5eac643e8b9786f3d894c310551fdcec9b69f&o=&hp=1'),
(16, 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/328137124.jpg?k=301ec735325c8156b3f17a8218ac9d51a1e5465ed5afa767f3cff8a767d36561&o=&hp=1'),
(16, 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/328137058.jpg?k=e87855fc1ee687f5747def2a2f43b0086986dab5cd97b945760f4dd0ced0142d&o=&hp=1'),

(17, 'https://i.szalas.hu/hotels/1023417/original/23291675.jpg'),
(17, 'https://meteor-turystyka.pl/images/base/2/1435/266349_40.jpg'),
(17, 'https://meteor-turystyka.pl/images/base/2/1435/266547_40.jpg'),

(18, 'https://example.com/guesthouse_r_image1.jpg'),
(18, 'https://example.com/guesthouse_r_image2.jpg'),
(18, 'https://example.com/guesthouse_r_image3.jpg'),

(19, 'https://example.com/hotel_s_image1.jpg'),
(19, 'https://example.com/hotel_s_image2.jpg'),
(19, 'https://example.com/hotel_s_image3.jpg'),

(20, 'https://example.com/motel_t_image1.jpg'),
(20, 'https://example.com/motel_t_image2.jpg'),
(20, 'https://example.com/motel_t_image3.jpg'),

(21, 'https://example.com/resort_u_image1.jpg'),
(21, 'https://example.com/resort_u_image2.jpg'),
(21, 'https://example.com/resort_u_image3.jpg'),

(22, 'https://example.com/inn_v_image1.jpg'),
(22, 'https://example.com/inn_v_image2.jpg'),
(22, 'https://example.com/inn_v_image3.jpg'),

(23, 'https://example.com/hostel_w_image1.jpg'),
(23, 'https://example.com/hostel_w_image2.jpg'),
(23, 'https://example.com/hostel_w_image3.jpg'),

(24, 'https://example.com/guesthouse_x_image1.jpg'),
(24, 'https://example.com/guesthouse_x_image2.jpg'),
(24, 'https://example.com/guesthouse_x_image3.jpg'),

(25, 'https://example.com/hotel_y_image1.jpg'),
(25, 'https://example.com/hotel_y_image2.jpg'),
(25, 'https://example.com/hotel_y_image3.jpg'),

(26, 'https://example.com/motel_z_image1.jpg'),
(26, 'https://example.com/motel_z_image2.jpg'),
(26, 'https://example.com/motel_z_image3.jpg'),

(27, 'https://example.com/resort_aa_image1.jpg'),
(27, 'https://example.com/resort_aa_image2.jpg'),
(27, 'https://example.com/resort_aa_image3.jpg'),

(28, 'https://example.com/inn_ab_image1.jpg'),
(28, 'https://example.com/inn_ab_image2.jpg'),
(28, 'https://example.com/inn_ab_image3.jpg'),

(29, 'https://example.com/hostel_ac_image1.jpg'),
(29, 'https://example.com/hostel_ac_image2.jpg'),
(29, 'https://example.com/hostel_ac_image3.jpg'),

(30, 'https://example.com/guesthouse_ad_image1.jpg'),
(30, 'https://example.com/guesthouse_ad_image2.jpg'),
(30, 'https://example.com/guesthouse_ad_image3.jpg'),

(31, 'https://example.com/hotel_ae_image1.jpg'),
(31, 'https://example.com/hotel_ae_image2.jpg'),
(31, 'https://example.com/hotel_ae_image3.jpg'),

(32, 'https://example.com/motel_af_image1.jpg'),
(32, 'https://example.com/motel_af_image2.jpg'),
(32, 'https://example.com/motel_af_image3.jpg'),

(33, 'https://example.com/resort_ag_image1.jpg'),
(33, 'https://example.com/resort_ag_image2.jpg'),
(33, 'https://example.com/resort_ag_image3.jpg'),

(34, 'https://example.com/inn_ah_image1.jpg'),
(34, 'https://example.com/inn_ah_image2.jpg'),
(34, 'https://example.com/inn_ah_image3.jpg'),

(35, 'https://example.com/hostel_ai_image1.jpg'),
(35, 'https://example.com/hostel_ai_image2.jpg'),
(35, 'https://example.com/hostel_ai_image3.jpg'),

(36, 'https://example.com/guesthouse_aj_image1.jpg'),
(36, 'https://example.com/guesthouse_aj_image2.jpg'),
(36, 'https://example.com/guesthouse_aj_image3.jpg'),

(37, 'https://example.com/hotel_ak_image1.jpg'),
(37, 'https://example.com/hotel_ak_image2.jpg'),
(37, 'https://example.com/hotel_ak_image3.jpg'),

(38, 'https://example.com/motel_al_image1.jpg'),
(38, 'https://example.com/motel_al_image2.jpg'),
(38, 'https://example.com/motel_al_image3.jpg'),

(39, 'https://example.com/resort_am_image1.jpg'),
(39, 'https://example.com/resort_am_image2.jpg'),
(39, 'https://example.com/resort_am_image3.jpg'),

(40, 'https://example.com/inn_an_image1.jpg'),
(40, 'https://example.com/inn_an_image2.jpg'),
(40, 'https://example.com/inn_an_image3.jpg'),

(41, 'https://example.com/hostel_ao_image1.jpg'),
(41, 'https://example.com/hostel_ao_image2.jpg'),
(41, 'https://example.com/hostel_ao_image3.jpg'),

(42, 'https://example.com/guesthouse_ap_image1.jpg'),
(42, 'https://example.com/guesthouse_ap_image2.jpg'),
(42, 'https://example.com/guesthouse_ap_image3.jpg'),

(43, 'https://example.com/hotel_aq_image1.jpg'),
(43, 'https://example.com/hotel_aq_image2.jpg'),
(43, 'https://example.com/hotel_aq_image3.jpg'),

(44, 'https://example.com/motel_ar_image1.jpg'),
(44, 'https://example.com/motel_ar_image2.jpg'),
(44, 'https://example.com/motel_ar_image3.jpg'),

(45, 'https://example.com/resort_as_image1.jpg'),
(45, 'https://example.com/resort_as_image2.jpg'),
(45, 'https://example.com/resort_as_image3.jpg');

(46, 'https://example.com/hotel_at_image1.jpg'),
(46, 'https://example.com/hotel_at_image2.jpg'),
(46, 'https://example.com/hotel_at_image3.jpg'),

(47, 'https://example.com/motel_au_image1.jpg'),
(47, 'https://example.com/motel_au_image2.jpg'),
(47, 'https://example.com/motel_au_image3.jpg'),

(48, 'https://example.com/resort_av_image1.jpg'),
(48, 'https://example.com/resort_av_image2.jpg'),
(48, 'https://example.com/resort_av_image3.jpg'),

(49, 'https://example.com/inn_aw_image1.jpg'),
(49, 'https://example.com/inn_aw_image2.jpg'),
(49, 'https://example.com/inn_aw_image3.jpg'),

(50, 'https://example.com/hostel_ax_image1.jpg'),
(50, 'https://example.com/hostel_ax_image2.jpg'),
(50, 'https://example.com/hostel_ax_image3.jpg');

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(1, 'Deluxe Suite', 3, 150.00, 75.00),
(1, 'Ocean View Room', 2, 120.00, 60.00),
(1, 'Executive Loft', 4, 180.00, 90.00),
(1, 'Royal Penthouse', 5, 250.00, 125.00),
(1, 'Family Suite', 6, 200.00, 100.00);

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(2, 'Beachfront Bungalow', 2, 200.00, 100.00),
(2, 'Poolside Villa', 4, 300.00, 150.00),
(2, 'Garden View Room', 2, 150.00, 75.00),
(2, 'Sunset Suite', 3, 250.00, 125.00),
(2, 'Coastal Cabin', 2, 100.00, 50.00);

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(3, 'Alpine Retreat', 2, 180.00, 90.00),
(3, 'Scenic Suite', 3, 220.00, 110.00),
(3, 'Lodge Room', 4, 200.00, 100.00),
(3, 'Mountain Chalet', 2, 160.00, 80.00),
(3, 'Valley View Loft', 3, 250.00, 125.00);

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(4, 'Antique Chamber', 2, 140.00, 70.00),
(4, 'Historic Suite', 3, 180.00, 90.00),
(4, 'Victorian Room', 2, 120.00, 60.00),
(4, 'Colonial Loft', 4, 200.00, 100.00),
(4, 'Heritage Suite', 2, 160.00, 80.00);

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(5, 'Skyline View Room', 2, 130.00, 65.00),
(5, 'Metropolis Suite', 3, 200.00, 100.00),
(5, 'Urban Loft', 4, 180.00, 90.00),
(5, 'Downtown Den', 2, 110.00, 55.00),
(5, 'Cityscape Chamber', 2, 120.00, 60.00);

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(6, 'Elegant Suite', 3, 170.00, 85.00),
(6, 'Classic Room', 2, 110.00, 55.00),

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(7, 'Regal Suite', 3, 190.00, 95.00),
(7, 'Executive Room', 2, 120.00, 60.00),
(7, 'Opulent Loft', 4, 240.00, 120.00),
(7, 'Majestic Chamber', 2, 160.00, 80.00),
(7, 'Royal Penthouse', 5, 300.00, 150.00);

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(8, 'Premium Suite', 3, 180.00, 90.00),
(8, 'Superior Room', 2, 130.00, 65.00),
(8, 'Deluxe Loft', 4, 230.00, 115.00),
(8, 'Imperial Chamber', 2, 140.00, 70.00),
(8, 'Sovereign Penthouse', 5, 290.00, 145.00);

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(9, 'Exclusive Suite', 3, 200.00, 100.00),
(9, 'Premier Room', 2, 140.00, 70.00),
(9, 'Elite Loft', 4, 250.00, 125.00),
(9, 'Noble Chamber', 2, 170.00, 85.00),
(9, 'Imperial Penthouse', 5, 310.00, 155.00);

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(10, 'Glamour Suite', 3, 210.00, 105.00),
(10, 'Chic Room', 2, 150.00, 75.00),
(10, 'Elite Loft', 4, 260.00, 130.00),
(10, 'Prestigious Chamber', 2, 180.00, 90.00),
(10, 'Sumptuous Penthouse', 5, 320.00, 160.00);

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(11, 'Exquisite Suite', 3, 220.00, 110.00),
(11, 'Stylish Room', 2, 160.00, 80.00),
(11, 'Prestige Loft', 4, 270.00, 135.00),
(11, 'Elegant Chamber', 2, 190.00, 95.00),
(11, 'Elite Penthouse', 5, 330.00, 165.00);

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(12, 'Royal Suite', 3, 230.00, 115.00),
(12, 'Elegant Room', 2, 170.00, 85.00),

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(13, 'Luxury Suite', 3, 240.00, 120.00),
(13, 'Grand Room', 2, 180.00, 90.00),
(13, 'Elite Loft', 4, 290.00, 145.00),
(13, 'Royal Chamber', 2, 210.00, 105.00),
(13, 'Supreme Penthouse', 5, 350.00, 175.00);

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(14, 'Premium Suite', 3, 250.00, 125.00),
(14, 'Superior Room', 2, 190.00, 95.00),
(14, 'Deluxe Loft', 4, 300.00, 150.00),
(14, 'Imperial Chamber', 2, 220.00, 110.00),
(14, 'Sovereign Penthouse', 5, 360.00, 180.00);

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(15, 'Exclusive Suite', 3, 260.00, 130.00),
(15, 'Premier Room', 2, 200.00, 100.00),
(15, 'Elite Loft', 4, 310.00, 155.00),
(15, 'Noble Chamber', 2, 230.00, 115.00),
(15, 'Imperial Penthouse', 5, 370.00, 185.00);

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(16, 'Glamour Suite', 3, 270.00, 135.00),
(16, 'Chic Room', 2, 210.00, 105.00),
(16, 'Elite Loft', 4, 320.00, 160.00),
(16, 'Prestigious Chamber', 2, 240.00, 120.00),
(16, 'Sumptuous Penthouse', 5, 380.00, 190.00);

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(17, 'Exquisite Suite', 3, 280.00, 140.00),
(17, 'Stylish Room', 2, 220.00, 110.00),
(17, 'Prestige Loft', 4, 330.00, 165.00),
(17, 'Elegant Chamber', 2, 250.00, 125.00),
(17, 'Elite Penthouse', 5, 390.00, 195.00);

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(18, 'Royal Suite', 3, 290.00, 145.00),
(18, 'Elegant Room', 2, 230.00, 115.00),

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(19, 'Luxury Suite', 3, 300.00, 150.00),
(19, 'Grand Room', 2, 240.00, 120.00),
(19, 'Elite Loft', 4, 350.00, 175.00),
(19, 'Royal Chamber', 2, 270.00, 135.00),
(19, 'Supreme Penthouse', 5, 410.00, 205.00);

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(20, 'Premium Suite', 3, 310.00, 155.00),
(20, 'Superior Room', 2, 250.00, 125.00),
(20, 'Deluxe Loft', 4, 360.00, 180.00),
(20, 'Imperial Chamber', 2, 280.00, 140.00),
(20, 'Sovereign Penthouse', 5, 420.00, 210.00);

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(21, 'Exclusive Suite', 3, 320.00, 160.00),
(21, 'Premier Room', 2, 260.00, 130.00),
(21, 'Elite Loft', 4, 370.00, 185.00),
(21, 'Noble Chamber', 2, 290.00, 145.00),
(21, 'Imperial Penthouse', 5, 430.00, 215.00);

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(22, 'Glamour Suite', 3, 330.00, 165.00),
(22, 'Chic Room', 2, 270.00, 135.00),
(22, 'Elite Loft', 4, 380.00, 190.00),
(22, 'Prestigious Chamber', 2, 300.00, 150.00),
(22, 'Sumptuous Penthouse', 5, 440.00, 220.00);

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(23, 'Exquisite Suite', 3, 340.00, 170.00),
(23, 'Stylish Room', 2, 280.00, 140.00),
(23, 'Prestige Loft', 4, 390.00, 195.00),
(23, 'Elegant Chamber', 2, 310.00, 155.00),
(23, 'Elite Penthouse', 5, 450.00, 225.00);

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(24, 'Royal Suite', 3, 350.00, 175.00),
(24, 'Elegant Room', 2, 290.00, 145.00),

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(25, 'Luxury Suite', 3, 360.00, 180.00),
(25, 'Grand Room', 2, 300.00, 150.00),
(25, 'Elite Loft', 4, 410.00, 205.00),
(25, 'Royal Chamber', 2, 330.00, 165.00),
(25, 'Supreme Penthouse', 5, 470.00, 235.00);

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(26, 'Premium Suite', 3, 370.00, 185.00),
(26, 'Superior Room', 2, 310.00, 155.00),
(26, 'Deluxe Loft', 4, 420.00, 210.00),
(26, 'Imperial Chamber', 2, 340.00, 170.00),
(26, 'Sovereign Penthouse', 5, 480.00, 240.00);

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(27, 'Exclusive Suite', 3, 380.00, 190.00),
(27, 'Premier Room', 2, 320.00, 160.00),
(27, 'Elite Loft', 4, 430.00, 215.00),
(27, 'Noble Chamber', 2, 350.00, 175.00),
(27, 'Imperial Penthouse', 5, 490.00, 245.00);

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(28, 'Glamour Suite', 3, 390.00, 195.00),
(28, 'Chic Room', 2, 330.00, 165.00),
(28, 'Elite Loft', 4, 440.00, 220.00),
(28, 'Prestigious Chamber', 2, 360.00, 180.00),
(28, 'Sumptuous Penthouse', 5, 500.00, 250.00);

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(29, 'Exquisite Suite', 3, 400.00, 200.00),
(29, 'Stylish Room', 2, 340.00, 170.00),
(29, 'Prestige Loft', 4, 450.00, 225.00),
(29, 'Elegant Chamber', 2, 370.00, 185.00),
(29, 'Elite Penthouse', 5, 510.00, 255.00);

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(30, 'Royal Suite', 3, 410.00, 205.00),
(30, 'Elegant Room', 2, 350.00, 175.00),

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(31, 'Luxury Suite', 3, 420.00, 210.00),
(31, 'Grand Room', 2, 360.00, 180.00),
(31, 'Elite Loft', 4, 470.00, 235.00),
(31, 'Royal Chamber', 2, 390.00, 195.00),
(31, 'Supreme Penthouse', 5, 530.00, 265.00);

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(32, 'Premium Suite', 3, 430.00, 215.00),
(32, 'Superior Room', 2, 370.00, 185.00),
(32, 'Deluxe Loft', 4, 480.00, 240.00),
(32, 'Imperial Chamber', 2, 400.00, 200.00),
(32, 'Sovereign Penthouse', 5, 540.00, 270.00);

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(33, 'Exclusive Suite', 3, 440.00, 220.00),
(33, 'Premier Room', 2, 380.00, 190.00),
(33, 'Elite Loft', 4, 490.00, 245.00),
(33, 'Noble Chamber', 2, 410.00, 205.00),
(33, 'Imperial Penthouse', 5, 550.00, 275.00);

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(34, 'Glamour Suite', 3, 450.00, 225.00),
(34, 'Chic Room', 2, 390.00, 195.00),
(34, 'Elite Loft', 4, 500.00, 250.00),
(34, 'Prestigious Chamber', 2, 420.00, 210.00),
(34, 'Sumptuous Penthouse', 5, 560.00, 280.00);

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(35, 'Exquisite Suite', 3, 460.00, 230.00),
(35, 'Stylish Room', 2, 400.00, 200.00),
(35, 'Prestige Loft', 4, 510.00, 255.00),
(35, 'Elegant Chamber', 2, 430.00, 215.00),
(35, 'Elite Penthouse', 5, 570.00, 285.00);

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(36, 'Royal Suite', 3, 470.00, 235.00),
(36, 'Elegant Room', 2, 410.00, 205.00),

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(37, 'Luxury Suite', 3, 480.00, 240.00),
(37, 'Grand Room', 2, 420.00, 210.00),
(37, 'Elite Loft', 4, 530.00, 265.00),
(37, 'Royal Chamber', 2, 450.00, 225.00),
(37, 'Supreme Penthouse', 5, 590.00, 295.00);

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(38, 'Premium Suite', 3, 490.00, 245.00),
(38, 'Superior Room', 2, 430.00, 215.00),
(38, 'Deluxe Loft', 4, 540.00, 270.00),
(38, 'Imperial Chamber', 2, 460.00, 230.00),
(38, 'Sovereign Penthouse', 5, 600.00, 300.00);

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(39, 'Exclusive Suite', 3, 500.00, 250.00),
(39, 'Premier Room', 2, 440.00, 220.00),
(39, 'Elite Loft', 4, 550.00, 275.00),
(39, 'Noble Chamber', 2, 470.00, 235.00),
(39, 'Imperial Penthouse', 5, 610.00, 305.00);

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(40, 'Glamour Suite', 3, 510.00, 255.00),
(40, 'Chic Room', 2, 450.00, 225.00),
(40, 'Elite Loft', 4, 560.00, 280.00),
(40, 'Prestigious Chamber', 2, 480.00, 240.00),
(40, 'Sumptuous Penthouse', 5, 620.00, 310.00);

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(41, 'Exquisite Suite', 3, 520.00, 260.00),
(41, 'Stylish Room', 2, 460.00, 230.00),
(41, 'Prestige Loft', 4, 570.00, 285.00),
(41, 'Elegant Chamber', 2, 490.00, 245.00),
(41, 'Elite Penthouse', 5, 630.00, 315.00);

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(42, 'Royal Suite', 3, 530.00, 265.00),
(42, 'Elegant Room', 2, 470.00, 235.00),

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(43, 'Luxury Suite', 3, 540.00, 270.00),
(43, 'Grand Room', 2, 480.00, 240.00),
(43, 'Elite Loft', 4, 590.00, 295.00),
(43, 'Royal Chamber', 2, 510.00, 255.00),
(43, 'Supreme Penthouse', 5, 650.00, 325.00);

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(44, 'Premium Suite', 3, 550.00, 275.00),
(44, 'Superior Room', 2, 490.00, 245.00),
(44, 'Deluxe Loft', 4, 600.00, 300.00),
(44, 'Imperial Chamber', 2, 520.00, 260.00),
(44, 'Sovereign Penthouse', 5, 660.00, 330.00);

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(45, 'Exclusive Suite', 3, 560.00, 280.00),
(45, 'Premier Room', 2, 500.00, 250.00),
(45, 'Elite Loft', 4, 610.00, 305.00),
(45, 'Noble Chamber', 2, 530.00, 265.00),
(45, 'Imperial Penthouse', 5, 670.00, 335.00);

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(46, 'Glamour Suite', 3, 570.00, 285.00),
(46, 'Chic Room', 2, 510.00, 255.00),
(46, 'Elite Loft', 4, 620.00, 310.00),
(46, 'Prestigious Chamber', 2, 540.00, 270.00),
(46, 'Sumptuous Penthouse', 5, 680.00, 340.00);

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(47, 'Exquisite Suite', 3, 580.00, 290.00),
(47, 'Stylish Room', 2, 520.00, 260.00),
(47, 'Prestige Loft', 4, 630.00, 315.00),
(47, 'Elegant Chamber', 2, 550.00, 275.00),
(47, 'Elite Penthouse', 5, 690.00, 345.00);

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(48, 'Royal Suite', 3, 590.00, 295.00),
(48, 'Elegant Room', 2, 530.00, 265.00),
(48, 'Imperial Loft', 4, 640.00, 320.00),
(48, 'Majestic Chamber', 2, 560.00, 280.00),
(48, 'Royal Penthouse', 5, 700.00, 350.00);

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(49, 'Luxury Suite', 3, 600.00, 300.00),
(49, 'Grand Room', 2, 540.00, 270.00),
(49, 'Elite Loft', 4, 650.00, 325.00),
(49, 'Royal Chamber', 2, 570.00, 285.00),
(49, 'Supreme Penthouse', 5, 710.00, 355.00);

INSERT INTO Rooms (hotelId, roomNumber, peopleCapacity, priceForPerson, childrenPrice)
VALUES 
(50, 'Premium Suite', 3, 610.00, 305.00),
(50, 'Superior Room', 2, 550.00, 275.00),
(50, 'Deluxe Loft', 4, 660.00, 330.00),
(50, 'Imperial Chamber', 2, 580.00, 290.00),
(50, 'Sovereign Penthouse', 5, 720.00, 360.00);