...
function SearchResults({ results, setData, destination, clicked, setSearchResult }: SearchResultsProps) {
  ...
  return (
    <div className="search-results absolute top-full mt-2 w-full bg-white shadow-md rounded-md border border-gray-300">
        <h3 className="text-lg font-semibold px-4 py-2 border-b border-gray-300">Search Results</h3>
        <div className="overflow-y-auto max-h-40">
            {results.map((result: string, index: number) => (
            <div key={index} className="px-4 py-2 hover:bg-gray-100 focus:bg-gray-100 cursor-pointer" onClick={handleClick}>
                <p>{result}</p>
            </div>
            ))}
        </div>
