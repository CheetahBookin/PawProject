        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <p><span className="font-semibold">Booked at: {reservation.createdAt.toString().split('T')[0]}</span></p>
              <p><span className="font-semibold">Check-in:</span> {reservation.fromDate.toString().split('T')[0]}</p>
              <p><span className="font-semibold">Check-out:</span> {reservation.toDate.toString().split('T')[0]}</p>
            </div>
            <div>
              <p><span className="font-semibold">Adults:</span> {reservation.adults}</p>
              <p><span className="font-semibold">Children:</span> {reservation.children}</p>
              <p><span className="font-semibold">Full price</span> {reservation.fullPrice}zł</p>
              <div className="flex gap-8">
                <div className={`font-semibold p-4 w-[6vw] flex justify-center ${reservation.paid ? `bg-emerald-600` : `bg-red-600`}`}>{reservation.paid ? "Paid" : "Not Paid"}</div>
                {!reservation.paid && <Link href={`/dashboard/reservations/checkout?order=${reservation.id}`} className="bg-brand-primary text-white px-4 py-2 rounded-md flex items-center">Pay</Link>}
              </div>
            </div>
          </div>
          <p className="mt-4"><span className="font-semibold">Hotel Type:</span> {hotel.type}</p>
        </>
      )}
      {!hotel && <p>Loading...</p>}
    </div>
